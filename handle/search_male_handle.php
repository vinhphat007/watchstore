...
if(isset($_GET['key-search'])) {
    $key = $_GET['key-search'];
    $sql = "SELECT p.*, c.*, p.name as name_product FROM product p JOIN category c ON p.category_id = c.id_category WHERE p.category_id=1 AND p.name LIKE '%" . $key . "%' ORDER BY p.id_product ASC LIMIT ". $limit ." OFFSET " . $start_from;
    $result = $db->query($sql);
    if ($result) {
        ...
        $sql2 = "SELECT p.*, c.*, p.name as name_product FROM product p JOIN category c ON p.category_id = c.id_category WHERE p.category_id=1 AND p.name LIKE '%" . $key . "%'";
